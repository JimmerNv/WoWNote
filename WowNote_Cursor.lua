--[[****************************************************************************
  * WowNote Cursor Effects                                                        *
  * Adapted from _Cursor 3.3.0.2 by Saiket and namespaced for WowNote.            *
  ****************************************************************************]]


local AddOnName = "WoWNote";
local Version = "3.3.0.2";
local WN = WowNote_Internal or {};

local function EnsureCursorDB ()
	if ( WN.InitDB ) then
		WN.InitDB();
	end
	...
	if ( type( WowNoteDB.cursorEffects ) ~= "table" ) then
		WowNoteDB.cursorEffects = {};
	end
	if ( type( WowNoteDB.cursorEffects.Sets ) ~= "table" ) then
		WowNoteDB.cursorEffects.Sets = {};
	end
	if ( type( WowNoteCharDB.cursorEffects ) ~= "table" ) then
		WowNoteCharDB.cursorEffects = {};
	end
	if ( type( WowNoteCharDB.cursorEffects.Cursors ) ~= "table" ) then
		WowNoteCharDB.cursorEffects.Cursors = {};
	end

	WowNoteCursorOptions = WowNoteDB.cursorEffects;
	WowNoteCursorCharacterOptions = WowNoteCharDB.cursorEffects;
	return WowNoteCursorOptions, WowNoteCursorCharacterOptions;
end

local L = WowNoteCursorLocalization;
local me = CreateFrame( "Frame", "WowNoteCursorEffectFrame" );
WowNoteCursor = me;
me.EnsureDB = EnsureCursorDB;

local function SetLegacyCursorBlocked ( Blocked )
	local Legacy = rawget( _G, "_Cursor" );
	if ( not Legacy or Legacy == me ) then
		return;
	end
	if ( Blocked and type( Legacy.BlockAdd ) == "function" ) then
		Legacy.BlockAdd( "WowNoteIntegration" );
	elseif ( not Blocked and type( Legacy.BlockRemove ) == "function" ) then
		Legacy.BlockRemove( "WowNoteIntegration" );
	end
end
...
--[[****************************************************************************
  * Function: WowNoteCursor.GetModel                                                 *
...
function me.GetModel ()
	...
	if ( not Model ) then
		...
		Model:SetAllPoints( nil ); -- Fullscreen, matching the original _Cursor viewport
		Model:Hide();
...
--[[****************************************************************************
  * Function: WowNoteCursor:ModelEnable                                              *
...
--[[****************************************************************************
  * Function: WowNoteCursor:ModelDisable                                             *
...
--[[****************************************************************************
  * Function: WowNoteCursor:ModelUpdate                                              *
...
--[[****************************************************************************
  * Function: WowNoteCursor.LoadSet                                                  *
  * Description: Unpacks a set into the current settings.                      *
  ****************************************************************************]]
do
	local Tables = {};
	function me.LoadSet ( Set )
		local Cursors = WowNoteCursorCharacterOptions.Cursors;
...
--[[****************************************************************************
  * Function: WowNoteCursor.SaveSet                                                  *
  * Description: Packs a set from the current settings.                        *
  ****************************************************************************]]
function me.SaveSet ( Set )
	wipe( Set );
	-- Pack data
	for Index, Cursor in ipairs( WowNoteCursorCharacterOptions.Cursors ) do
...
--[[****************************************************************************
  * Function: WowNoteCursor.Update                                                   *
  * Description: Resynchronizes WowNoteCursor with its settings table.               *
  ****************************************************************************]]
do
	local StrataLevels = {
...
do
	local Blocks = {};
--[[****************************************************************************
  * Function: WowNoteCursor.BlockAdd                                                 *
...
--[[****************************************************************************
  * Function: WowNoteCursor.BlockRemove                                              *
...
--[[****************************************************************************
  * Function: WowNoteCursor:ADDON_LOADED                                             *
  ****************************************************************************]]
do
	...
	function me:ADDON_LOADED ( _, AddOn )
		if ( AddOn and AddOn:lower() == AddOnName:lower() and not Initialized ) then
			Initialized = true;

			local Account, Character = EnsureCursorDB();
			local LegacyAccount = rawget( _G, "_CursorOptions" );
			local LegacyCharacter = rawget( _G, "_CursorOptionsCharacter" );
			if ( Account.Version == nil and next( Account.Sets ) == nil and type( LegacyAccount ) == "table" ) then
				WowNoteDB.cursorEffects = CopyTable( LegacyAccount );
				Account, Character = EnsureCursorDB();
			end
			if ( not Character.Cursors[ 1 ] and type( LegacyCharacter ) == "table" and type( LegacyCharacter.Cursors ) == "table" and LegacyCharacter.Cursors[ 1 ] ) then
				WowNoteCharDB.cursorEffects = CopyTable( LegacyCharacter );
				Account, Character = EnsureCursorDB();
			end
			if ( Account.Version == nil and next( Account.Sets ) == nil ) then
				Account.Sets = CopyTable( me.DefaultSets );
			end

			if ( Account.Version ~= Version ) then
				if ( VersionCompare( Account.Version, "3.1.0.2" ) < 0 ) then
					local Name = L.SETS[ "Face Melter (Warning, bright!)" ];
					if ( Account.Sets[ Name ] ) then
						Account.Sets[ Name ] = CopyTable( me.DefaultSets[ Name ] );
					end
				end
			...
			if ( not Character.Cursors[ 1 ] ) then
			...
			if ( WowNote_IsModuleEnabled ) then
				Enabled = WowNote_IsModuleEnabled( "cursorEffects" );
			end
			WowNote_SetCursorEffectsModuleEnabled( Enabled );
		elseif ( AddOn and AddOn:lower() == "_cursor" ) then
			SetLegacyCursorBlocked( WowNote_IsCursorEffectsModuleEnabled() );
		end
	end
end
--[[****************************************************************************
  * Function: WowNoteCursor:OnEvent                                                  *
