...
local function CanEdit()
    local db = EnsureDB()
    return db.testMode == true or IsLeaderOrAssistant()
end
...
local function AddRow()
    if not CanEdit() then Print("Only raid leaders or assistants can edit the bite order."); return end
    local rows = ActiveRows()
    EnsureFirstRow(rows)
...
local function CreateEditor()
    ...
    editorFrame.testButton:SetPoint("TOPRIGHT", editorFrame, "TOPRIGHT", -330, -18)
    ...
    editorFrame.deleteRow:SetScript("OnClick", function() if CanEdit() then local rows = ActiveRows(); if table.getn(rows) > 1 then table.remove(rows) end; WowNote_BiteHelper_RefreshEditor() end end)
...
function WowNote_BiteHelper_RefreshEditor()
    ...
    editorFrame.status:SetText(db.testMode and "|cffff4040TEST MODE - local simulation only|r" or (CanEdit() and "Raid leader/assistant editing enabled" or "Read-only: raid lead or assist required"))
    ...
    SetButtonEnabled(editorFrame.post, canEdit)
